			...
			<table cellpadding="0" cellspacing="0" width="100%" class="list">
				...
				<?php foreach ($Grade as $v):?>
					<tr>
						...
						<td><?=$v->OWNER?></td>
						<td><?=$v->UPDATETIME?></td>
						<td><input type="button" class="gedit" data-id="<?=$v->ID?>" value="編輯"></td>
					</tr>
				<?php endforeach; ?>
				</tbody>
			</table>
	...
	$.ajax({
		...
		success:function(rs){
			...
			for (var i in rs){
				...
				html+= '<tr '+g_sel+'><td class="name"><a href="javascript:void(0)" class="gc" data-id="'+rs[i].ID+'">'+rs[i].NAME+'</a></td><td>'+rs[i].OWNER+'</td><td>'+rs[i].UPDATETIME+'</td><td><input type="button" class="gedit" data-id="'+rs[i].ID+'" value="編輯"></td></tr>';
			}
	...
	$.ajax({
		...
		success:function(rs){
			...
			for (var i in rs){
				...
				html+= '<tr '+s_sel+'><td class="name"><a href="javascript:void(0)" class="sc" data-id="'+rs[i].ID+'">'+rs[i].NAME+'</a></td><td>'+rs[i].OWNER+'</td><td>'+rs[i].UPDATETIME+'</td><td><input type="button" class="sedit" data-id="'+rs[i].ID+'" value="編輯"></td></tr>';
			}
			alert('新增成功');
...
function nchap(){
	...
	$.ajax({
		...
		success:function(rs){
			...
			var html = '';
			for (var i in rs){
				html+= '<tr><td class="name"><a href="javascript:void(0)" class="cc" data-id="'+rs[i].ID+'">'+rs[i].NAME+'</a></td><td>'+rs[i].OWNER+'</td><td>'+rs[i].UPDATETIME+'</td><td><input type="button" class="cedit" data-id="'+rs[i].ID+'" value="編輯"></td></tr>';
			}
			alert('新增成功');
...
function getchap(v){
	...
	$.ajax({
		...
		success: function(rs){
			...
			var html = '';
			for (var i in rs){
				html+= '<tr><td class="name"><a class="cc" data-id="'+rs[i].ID+'">'+rs[i].NAME+'</a></td><td>'+rs[i].OWNER+'</td><td>'+rs[i].UPDATETIME+'</td><td><input type="button" class="cedit" data-id="'+rs[i].ID+'" value="編輯"></td></tr>';
			}
			gb('chaplist').innerHTML = html;
