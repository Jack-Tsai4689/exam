...
<div id="all">
    ...
    <div name="part" id="part">
        ...
        <div class="title_intro">
            ...
            <div><input type="button" class="btn w100" value="新增題目" onclick='window.open("<?=site_url('/question/create')?>","_blank","width=800,height=600,resizable=yes,scrollbars=yes,location=no");'>　<input type="button" class="btn w100 ware" value="從題庫加入"></div>
...
function save_part(){
    ...
    $.ajax({
        type:"POST",
        url:'<?=site_url('/sets/update_setsort') ?>',
        dataType:'json',
...
function moreone(){
    ...
    //     )
    // );
    var html = '<tr name="nsub'+add+'"><td align="left"><label class="f17">新大題</label></td><td><img src="<?=base_url('/images/icon_op_f.png')?>" onclick="update_recover('+add+')" width="15" height="15" class="sub_update_del"><input type="hidden" name="sub[]" value=""></td></tr><tr name="nsub'+add+'" class="deep"><td align="right">分數比重</td><td><input type="text" name="sub_score[]" maxlength="4" style="width:40px; text-align:center;" class="input_field">%</td></tr><tr name="nsub'+add+'" class="shallow"><td align="right">翻頁控制</td><td><select name="sub_control[]"><option value="Y">可回上題修改</option><option value="N">不可回上題修改</option></select></td></tr><tr name="nsub'+add+'" class="deep"><td align="right" style="vertical-align:top;">大題說明</td><td><textarea name="sub_intro[]" placeholder="選擇題"></textarea></td></tr>';
    $(gb('more')).append(html);
}
function edit_sub(){
    $(gb('more')).html('');
    $.ajax({
        ...
        success: function(data, textStatus, jqXHR){
            ...
            for (var i in data){
                ...
                var j = Number(i)+1;
                html+= '<tr name="sub'+data[i].sid+'"><td align="left"><label class="f17">大題'+j+'</label></td><td><img src="<?=base_url('/images/icon_op_f.png')?>" onclick="update_recover('+data[i].sid+')" width="15" height="15" class="sub_update_del"><input type="hidden" name="sub[]" value="'+data[i].sid+'"></td></tr><tr name="sub'+data[i].sid+'" class="deep"><td align="right">分數比重</td><td><input type="text" name="sub_score[]" value="'+data[i].percen+'" maxlength="4" style="width:40px; text-align:center;" class="input_field">%</td></tr><tr name="sub'+data[i].sid+'" class="shallow"><td align="right">翻頁控制</td><td><select name="sub_control[]"><option '+con_Y+' value="Y">可回上題修改</option><option '+con_N+' value="N">不可回上題修改</option></select></td></tr><tr name="sub'+data[i].sid+'" class="deep"><td align="right" style="vertical-align:top;">大題說明</td><td><textarea name="sub_intro[]" placeholder="選擇題" value='+data[i].intro+'>'+data[i].intro+'</textarea></td></tr>';
            };
            $(gb('more')).html(html);
        }
    });
